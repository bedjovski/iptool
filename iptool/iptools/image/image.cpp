...
void image::setPixel(const int row, const int col, const int rgb, const int value)
{
	switch (rgb) 
	{
		case 1: data.redChannel [row*data.numColumns+col] = value; break;
		case 2: data.greenChannel [row*data.numColumns+col] = value; break;
		case 3: data.blueChannel [row*data.numColumns+col] = value; break;
	}
}
...
int image::getPixel(const int row, const int col, const int rgb) 
{
	...
	switch (rgb)
	{
		case 1: value = data.redChannel [row*data.numColumns+col]; break;
		case 2: value = data.greenChannel [row*data.numColumns+col]; break;
		case 3: value = data.blueChannel [row*data.numColumns+col]; break;
	}
	return value;
}
